...
const SectionShell = ({
  ...
  return (
    <View style={composeStyles(sectionStyle)}>
      {showHeading && (
        <Heading style={composeStyles(sectionHeadingStyle)}>
          {sectionTitle}
        </Heading>
      )}
      {children}
...
const SectionItems = ({
  ...
  const rtlRowStyle = rtl
    ? { flexDirection: "row-reverse" as const }
    : undefined;

  if (!useTimeline) {
    if (layout.isGrid) {
      const rows = getSectionItemRows(
...
const SectionItem = ({
  ...
  if (!useTimeline) {
    return (
      <Div style={composeStyles(itemStyle, sectionItemStyle, style)}>
        {children}
      </Div>
    );
  }
  ...
  return (
    <View style={composeStyles(timelineItemStyle)}>
      ...
      <Div style={composeStyles(itemStyle, timelineContentStyle, style)}>
        {children}
      </Div>
    </View>
  );
    ...
    <SectionShell sectionId={sectionId} title={experience.title}>
      <SectionItems columns={experience.columns}>
        {items.map((item) => {
          ...
          const { top: headerLocation, bottom: headerPeriod } =
          ...
          const renderInlineHeader = () => (
            <InlineItemHeader
              ...
              trailing={
                <Text style={composeStyles(alignRightStyle)}>
                  {item.period}
                </Text>
              }
            />
  ...
  return (
    <SectionShell sectionId={sectionId} title={education.title}>
      <SectionItems columns={education.columns}>
        {items.map((item) => {
          const degreeAndGrade = [item.degree, item.grade]
            .filter(Boolean)
            .join(" • ");
          const locationAndPeriod = [item.location, item.period]
            .filter(Boolean)
            .join(" • ");
          const gradeAndLocation = [item.grade, item.location]
            .filter(Boolean)
            .join(" • ");
              ...
              <SectionItemHeader>
                {inlineItemHeader ? (
                  <InlineItemHeader
                    ...
                    middle={
                      <ItemTitle website={item.website}>
                        {item.organization}
                      </ItemTitle>
                    }
                    trailing={
