...
**Page**, and **Picture** settings. They use Reactive Resume Semantic CSS (RRSS), a CSS-like language designed for
resume PDFs.

<Note>
  RRSS styles the PDF output, not the browser interface. It cannot load fonts, images, scripts, or other resources, and
...
<Steps>
  ...
  <Step title="Review the converted draft">
    Check the preview and warnings below the editor. The draft starts with `@rr-version 1;`.
  </Step>
...
<!-- RRSS-EXAMPLE:valid FIRST_CHANGE -->
```css
@rr-version 1;
...
The first line tells Reactive Resume which language version the stylesheet uses. Keep `@rr-version 1;` at the start of
every stylesheet.
...
RRSS selectors describe resume content rather than a template's internal HTML. Selector and attribute names are
...
RRSS supports selector lists, descendant (` `), child (`>`), adjacent sibling (`+`), and general sibling (`~`)
combinators. It also supports `:root`, `:first-child`, `:last-child`, `:only-child`, `:is()`, `:where()`, `:not()`,
`:nth-child()`, and `:nth-of-type()`.

<!-- RRSS-EXAMPLE:valid TARGETING -->
```css
@rr-version 1;
...
RRSS exposes the resolved builder settings as read-only `--rr-*` variables. Define your own variables in `:root`, then
reuse the builder values instead of duplicating colors or dimensions.

<!-- RRSS-EXAMPLE:valid BUILDER_VALUES -->
```css
@rr-version 1;
...
assign a value to a `--rr-*` variable; create an author variable such as `--accent` instead.

| Builder setting | Read-only variables |
| --- | --- |
| Colors | `--rr-primary-color`, `--rr-text-color`, `--rr-background-color` |
| Typography | `--rr-body-font-size`, `--rr-body-line-height`, `--rr-heading-font-size`, `--rr-heading-line-height` |
| Page and layout | `--rr-page-gap-x`, `--rr-page-gap-y`, `--rr-page-margin-x`, `--rr-page-margin-y`, `--rr-page-width`, `--rr-page-height`, `--rr-sidebar-width` |
| Picture | `--rr-picture-size`, `--rr-picture-rotation`, `--rr-picture-aspect-ratio`, `--rr-picture-border-radius`, `--rr-picture-border-width`, `--rr-picture-border-color`, `--rr-picture-shadow-width`, `--rr-picture-shadow-color` |

Use `pt` for predictable PDF spacing and type sizes. RRSS also accepts `px`, `in`, `mm`, `cm`, `%`, `vw`, `vh`, `em`,
...
| Picture treatment | `object-fit`, `object-position`, `-rr-shadow-color`, `-rr-shadow-width` |
| PDF structure | `break-before`, `break-inside`, `orphans`, `widows`, `-rr-min-presence-ahead`, `size` |

Use `display: none` only to hide an existing semantic node. RRSS cannot add, remove, duplicate, or re-parent resume
data.
...
<!-- RRSS-EXAMPLE:valid RICH_TEXT_LISTS -->
```css
@rr-version 1;
...
<!-- RRSS-EXAMPLE:valid ITEM_FIELDS -->
```css
@rr-version 1;
...
<!-- RRSS-EXAMPLE:valid TEMPLATE_PART -->
```css
@rr-version 1;
...
<!-- RRSS-EXAMPLE:valid PDF_MEDIA_QUERY -->
```css
@rr-version 1;
...
RRSS does not support classes, pseudo-elements, CSS Grid, arbitrary at-rules, `@import`, `@font-face`, `url()`,
