...
- 40 implementation PRs created by this audit: 38 merged by the repository owner; #3448, #3449 remain open. Audit-only documentation PRs #3418 and #3440 are excluded and are both merged.
...
- In progress: paragraph indentation PR #3448, ordered-list marker PR #3449, Unicode-space preservation discovered during #3093, and remaining issue reproductions. RTL canvas PR #3447 is owner-merged. Bullet pagination (#3344), Ditgar alignment (#3068), and Arabic preview centering (#2745) are owner-merged. Reporter confirmed #3433 no longer reproduces after restarting their setup; issue closed without an attributed code fix. #3196 remains open because merged #3438 addressed a separate content-loss regression, not missing table borders. GitHub state refreshed against the current open-issue and PR inventories.
...
| [#3275](https://github.com/amruthpillai/reactive-resume/issues/3275) | [#3447](https://github.com/amruthpillai/reactive-resume/pull/3447) | Head `bdb7abb3b`: fixes inherited canvas drawing direction while preserving resume and interface DOM direction. Production baseline has two Arabic-resume failures with 35,009 differing pixels and two English-resume controls passing; final eight browser cases verify exact PDF raster parity and preview centering. All 777 web tests, build, typecheck, boundaries and repository checks pass. Independent review clean; all CI checks pass, including 34 browser scenarios. Broader exported-PDF report remains open. |
| [#3397](https://github.com/amruthpillai/reactive-resume/issues/3397) | [#3448](https://github.com/amruthpillai/reactive-resume/pull/3448) | Head `6cfb00387` (external rebase and DOCX quote/list inset follow-up): adds approved whole-paragraph/heading indentation through existing controls, with persisted levels and PDF/DOCX export. PDF reserves at least half the available block width. All 945 PDF, 68 DOCX and 177 affected web tests pass, along with build, three typechecks and boundaries. Production authoring/save/reload/export and an independent 16-case narrow-width matrix pass; LTR/RTL continuation-page tests retain text and inset. Earlier head `421f60e98` passed all CI gates, including 30 browser scenarios. The external quote/list fix has red/green XML coverage and a fresh 68-test DOCX pass; refreshed CI is running. Literal leading whitespace/tabs and existing equivalent-width overlong-word clipping remain outside this implementation. |
| [#2751](https://github.com/amruthpillai/reactive-resume/issues/2751) | [#3449](https://github.com/amruthpillai/reactive-resume/pull/3449) | Head `d834df6ce`: prevents ordered-list markers from overlapping body text, with a common gutter based on digit count, resolved font size and letter spacing. All 932 PDF tests, including 30 new marker and 36 existing pagination cases, typecheck, boundaries and repository checks pass. Independent review caught and verified the custom-letter-spacing correction. Direct/inherited styles, fonts, digit transitions, RTL, columns and page breaks are covered. Original missing leading digit is unproven; nested RTL list flattening matches unchanged baseline and remains separate. |
...
- #3397: approved — indent the whole paragraph through the existing controls; implemented in #3448, pending review and merge.
...
- Review PR #3448; do not merge from this task. Literal leading spaces/tabs remain outside the implemented whole-paragraph alternative. Bounded PDF inset and equivalent-width overlong-word behavior are documented.

**Implementation:** [PR #3448](https://github.com/amruthpillai/reactive-resume/pull/3448). Head `6cfb00387` (external rebase and DOCX quote/list inset follow-up): adds approved whole-paragraph/heading indentation through existing controls, with persisted levels and PDF/DOCX export. PDF reserves at least half the available block width. All 945 PDF, 68 DOCX and 177 affected web tests pass, along with build, three typechecks and boundaries. Production authoring/save/reload/export and an independent 16-case narrow-width matrix pass; LTR/RTL continuation-page tests retain text and inset. Earlier head `421f60e98` passed all CI gates, including 30 browser scenarios. The external quote/list fix has red/green XML coverage and a fresh 68-test DOCX pass; refreshed CI is running. Literal leading whitespace/tabs and existing equivalent-width overlong-word clipping remain outside this implementation.
...
**Implementation:** [PR #3447](https://github.com/amruthpillai/reactive-resume/pull/3447). Head `bdb7abb3b`: fixes inherited canvas drawing direction while preserving resume and interface DOM direction. Production baseline has two Arabic-resume failures with 35,009 differing pixels and two English-resume controls passing; final eight browser cases verify exact PDF raster parity and preview centering. All 777 web tests, build, typecheck, boundaries and repository checks pass. Independent review clean; all CI checks pass, including 34 browser scenarios. Broader exported-PDF report remains open.
...
- Implement and review a separate scoped ideographic-space preservation fix with actual-PDF baseline/green and ordinary ASCII whitespace controls. Reference this issue without closing it; the generic Unicode-space defect is not proven to match the original screenshot.
...
- Review PR #3449 and its CI; do not merge from this task. Keep historical missing-digit report open until exact source data establishes equivalence. Existing nested RTL flattening remains separately documented.

**Implementation:** [PR #3449](https://github.com/amruthpillai/reactive-resume/pull/3449). Head `d834df6ce`: prevents ordered-list markers from overlapping body text, with a common gutter based on digit count, resolved font size and letter spacing. All 932 PDF tests, including 30 new marker and 36 existing pagination cases, typecheck, boundaries and repository checks pass. Independent review caught and verified the custom-letter-spacing correction. Direct/inherited styles, fonts, digit transitions, RTL, columns and page breaks are covered. Original missing leading digit is unproven; nested RTL list flattening matches unchanged baseline and remains separate.
