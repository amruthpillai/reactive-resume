...
- Custom (user-built) templates are a separate surface from the built-in templates above: schema in `packages/schema/src/custom-template.ts`, API in `packages/api/src/features/custom-template/`, the drag-and-drop editor in `apps/web/src/routes/template-editor/$templateId` + `apps/web/src/features/template-editor`, and the PDF renderer in `packages/pdf/src/templates/custom/`. A custom template is a node tree rendered through its chosen base template's real styles; those styles are exposed to the editor's HTML canvas via the `@reactive-resume/pdf/template-styles` export. Keep `nodeStyleToReactPdf` (PDF) and the canvas `nodeStyleToCss` translators in lockstep.
